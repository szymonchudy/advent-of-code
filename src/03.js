...
input
  ...
  .forEach((sack) => {
    ...
    const letter = intersection
      .filter((element, index) => intersection.indexOf(element) === index)[0];

    if (letter) {
      part1 += letters[letter];
    }
  });
...
input
  ...
  .forEach(([elf1, elf2, elf3]) => {
    const intersection = elf1
      .filter((x) => elf2.includes(x) && elf3.includes(x));
    const letter = intersection
      .filter((element, index) => intersection.indexOf(element) === index)[0];

    if (letter) {
      part2 += letters[letter];
    }
  });

console.log('Day 03:', { part1, part2 });
